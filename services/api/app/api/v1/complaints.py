...
ALLOWED_TYPES = {"image/jpeg": ("JPEG", "jpg"), "image/png": ("PNG", "png"), "image/webp": ("WEBP", "webp")}
...
async def read_and_validate_image(image: UploadFile, max_size: int) -> tuple[bytes, str, str]:
    content_type = image.content_type or ""
    if content_type not in ALLOWED_TYPES:
        raise HTTPException(status_code=415, detail="Only JPEG, PNG, and WebP images are allowed")
    data = await image.read(max_size + 1)
    ...
    expected_format, extension = ALLOWED_TYPES[content_type]
...
def complaint_response(complaint: Complaint) -> dict:
    return {
        ...
        "location": {"latitude": complaint.latitude, "longitude": complaint.longitude, "label": complaint.location_label},
        "images": [
            {"id": image.id, "object_key": image.object_key, "filename": image.original_filename, "content_type": image.content_type, "size_bytes": image.size_bytes}
            for image in complaint.images
        ],
        "history": [
            {"from_status": entry.from_status, "to_status": entry.to_status, "created_at": entry.created_at}
...
async def create_complaint(
    ...
    location_label = location_label.strip()[:255] or None if location_label else None

    image_data = None
    content_type = None
    extension = None
    if image is not None:
        image_data, content_type, extension = await read_and_validate_image(
            image, get_settings().max_complaint_image_size_bytes
        )
...
@router.get("")
async def list_complaints(user: User = Depends(require_roles(RoleName.STUDENT)), db: AsyncSession = Depends(get_db)) -> list[dict]:
    result = await db.scalars(
        select(Complaint)
        .options(selectinload(Complaint.images), selectinload(Complaint.status_history))
        .where(Complaint.student_id == user.id)
        .order_by(Complaint.created_at.desc())
    )
    return [complaint_response(complaint) for complaint in result.all()]


@router.get("/{complaint_id}")
async def get_complaint(complaint_id: str, user: User = Depends(require_roles(RoleName.STUDENT)), db: AsyncSession = Depends(get_db)) -> dict:
    return complaint_response(await get_student_complaint(complaint_id, user, db))
